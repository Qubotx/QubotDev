...
class Controller:
    ...
    def __init__(self, port="/dev/ttyUSB0"):
        ...
        self.client = ModbusClient(
            method="rtu", port=self._port, baudrate=115200, timeout=1
        )
    ...
    def disable_motor(self):
        result = self.client.write_register(
            self.CONTROL_REG, self.DOWN_TIME, unit=self.ID
        )
    ...
    def clear_alarm(self):
        result = self.client.write_register(
            self.CONTROL_REG, self.ALRM_CLR, unit=self.ID
        )
    ...
    def set_accel_time(self, L_ms, R_ms):
        ...
        result = self.client.write_registers(
            self.L_ACL_TIME, [int(L_ms), int(R_ms)], unit=self.ID
        )
    ...
    def set_decel_time(self, L_ms, R_ms):
        ...
        result = self.client.write_registers(
            self.L_DCL_TIME, [int(L_ms), int(R_ms)], unit=self.ID
        )
    ...
    def set_rpm(self, L_rpm, R_rpm):
        ...
        result = self.client.write_registers(
            self.L_CMD_RPM, [left_bytes, right_bytes], unit=self.ID
        )
        return result
    ...
    def set_maxRPM_pos(self, max_L_rpm, max_R_rpm):
        ...
        result = self.client.write_registers(
            self.L_MAX_RPM_POS, [int(max_L_rpm), int(max_R_rpm)], unit=self.ID
        )

    def set_position_async_control(self):

        result = self.client.write_register(
            self.POS_CONTROL_TYPE, self.ASYNC, unit=self.ID
        )

    def move_left_wheel(self):

        result = self.client.write_register(
            self.CONTROL_REG, self.POS_L_START, unit=self.ID
        )

    def move_right_wheel(self):

        result = self.client.write_register(
            self.CONTROL_REG, self.POS_R_START, unit=self.ID
        )
    ...
    def set_relative_angle(self, ang_L, ang_R):
        ...
        result = self.client.write_registers(
            self.L_CMD_REL_POS_HI, all_cmds_array, unit=self.ID
        )
